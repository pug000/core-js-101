 ...
 */
const extractNameFromTemplate = (value) => value.replace(/.*.,.|!/gi, '');
...
const getRectangleString = (width, height) => {
  const newWidth = '─'.repeat(width - 2);
  const newHeight = `│${' '.repeat(width - 2)}│\n`.repeat(height - 2);
  return `┌${newWidth}┐\n${newHeight}└${newWidth}┘\n`;
};
 ...
 *
 */
const encodeToRot13 = (str) => str.replace(/[A-Z]/gi, (char) => 'NOPQRSTUVWXYZABCDEFGHIJKLMnopqrstuvwxyzabcdefghijklm'[
